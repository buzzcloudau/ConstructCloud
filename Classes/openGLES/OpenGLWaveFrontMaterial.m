...
+ (id)defaultMaterial
{
	return [[OpenGLWaveFrontMaterial alloc] initWithName:@"default" 
												shininess:65.0 
												 ...
												 specular:Color3DMake(0.0, 0.0, 0.0, 1.0)];
	
}
+ (id)materialsFromMtlFile:(NSString *)path
{
	NSMutableDictionary *ret = [NSMutableDictionary dictionary];
	ret[@"default"] = [OpenGLWaveFrontMaterial defaultMaterial];
	
	NSError *error = nil;
	NSStringEncoding *encoding = nil;
	
	NSString *mtlData = [NSString stringWithContentsOfFile:path usedEncoding:encoding error:&error];
		...
		if ([line hasPrefix:@"newmtl"]) // Start of new material
		{
			...
			for (int j = i; j <= mtlEnd; j++)
			{
				...
				else if ([parseLine hasPrefix:@"Ka "])  // CIEXYZ currently not supported, must be specified as RGB
				{
					...
					material.ambient = Color3DMake([colorParts[0] floatValue], [colorParts[1] floatValue], [colorParts[2] floatValue], 1.0);
				}
				else if ([parseLine hasPrefix:@"Kd "])
				{
					NSArray *colorParts = [[parseLine substringFromIndex:3] componentsSeparatedByCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
					material.diffuse =  Color3DMake([colorParts[0] floatValue], [colorParts[1] floatValue], [colorParts[2] floatValue], 1.0);
				}
				else if ([parseLine hasPrefix:@"Ks "])
				{
					NSArray *colorParts = [[parseLine substringFromIndex:3] componentsSeparatedByCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
					material.specular =  Color3DMake([colorParts[0] floatValue], [colorParts[1] floatValue], [colorParts[2] floatValue], 1.0);
				}
				else if ([parseLine hasPrefix:@"map_Kd "])
				{
					glEnableClientState(GL_TEXTURE);
					NSString *texName = [parseLine substringFromIndex:7];
					NSString *baseName = [texName componentsSeparatedByString:@"."][0];

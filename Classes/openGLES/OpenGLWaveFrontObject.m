...
- (id)initWithPath:(NSString *)path
{
	...
	if ((self = [super init]))
	{
		...
		NSString *objData = [NSString stringWithContentsOfFile:path usedEncoding:enc error:&error];
		...
		for (NSString * line in lines)
		{
			...
			else if ([line hasPrefix:@"f"])
			{
				...
				for (NSString *oneFace in faces)
				{
					...
					NSString *faceKey = [NSString stringWithFormat:@"%@/%@", faceParts[0], ([faceParts count] > 1) ? faceParts[1] : 0];
		...
		for (NSString * line in lines)
		{
			if ([line hasPrefix:@"v "])
			{
				...
				vertices[vertexCount].x = [lineVertices[0] floatValue];
				vertices[vertexCount].y = [lineVertices[1] floatValue];
				vertices[vertexCount].z = [lineVertices[2] floatValue];
...
//					else
			...
			else if ([line hasPrefix:@"g "])
			{
				...
				{
					NSString *nextLine = lines[counter++];
				...
				OpenGLWaveFrontMaterial *material = materials[materialName] ;
				if (material == nil)
			...
			else if ([line hasPrefix:@"usemtl "])
			{
				...
				currentGroup.material = materials[materialKey];
			}
			else if ([line hasPrefix:@"f "])
			{
				NSString *lineTrunc = [line substringFromIndex:2];
				...
				NSArray *vertex1Parts = [faceIndexGroups[0] componentsSeparatedByString:@"/"];
				GLuint vertex1Index = [vertex1Parts[kGroupIndexVertex] intValue]-1;
				GLuint vertex1TextureIndex = 0;
				if ([vertex1Parts count] > 1)
					vertex1TextureIndex = [vertex1Parts[kGroupIndexTextureCoordIndex] intValue]-1;
				if (rootNode == NULL)
				...
				NSArray *vertex2Parts = [faceIndexGroups[1] componentsSeparatedByString:@"/"];
				processOneVertex(rootNode, [vertex2Parts[kGroupIndexVertex] intValue]-1, [vertex2Parts count] > 1 ? [vertex2Parts[kGroupIndexTextureCoordIndex] intValue]-1 : 0, &vertexCount, vertices, allTextureCoords, textureCoords, valuesPerCoord, &currentGroup.faces[groupFaceCount].v2);	
				NSArray *vertex3Parts = [faceIndexGroups[2] componentsSeparatedByString:@"/"];
				processOneVertex(rootNode, [vertex3Parts[kGroupIndexVertex] intValue]-1, [vertex3Parts count] > 1 ? [vertex3Parts[kGroupIndexTextureCoordIndex] intValue]-1 : 0, &vertexCount, vertices, allTextureCoords, textureCoords, valuesPerCoord, &currentGroup.faces[groupFaceCount].v3);
...
- (void)calculateNormals
{
	...
	{
		OpenGLWaveFrontGroup *oneGroup = groups[i];
